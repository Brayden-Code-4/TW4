...
Paste this file into Dev.to, Hashnode or Medium. Keep the canonical URL so the Docusaurus post stay the main copy.
...
sequenceDiagram
  ...
  C->>S: POST /api/v1/tasks + X-API-Key
  S->>S: requireApiKey
  alt missing or wrong key
    S-->>C: 401 unauthorized
  else API_KEY empty in env
    S-->>C: 500 server_misconfigured
  else ok
    S->>S: parse JSON and validate title
    S->>F: load array, push task, save
    S-->>C: 201 + data
  end
```

GET `/health` and GET `/api/v1/tasks` skip the key. That is a product choice: you can health-check and list without leaking the secret into every CI ping. Writes cannot.
